<x-layouts.mobile-app :currentPage="'create'">
    <div class="min-h-screen bg-gradient-to-br from-primary/5 via-background to-secondary/5 px-4 py-6">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('dashboard') }}" class="w-10 h-10 rounded-full bg-surface flex items-center justify-center shadow-card">
                <span class="material-symbols-rounded">close</span>
            </a>
        </div>

        <form method="POST" action="{{ route('transactions.store') }}" id="transactionForm" x-data="transactionForm()" @submit="validateForm($event)">
            @csrf
